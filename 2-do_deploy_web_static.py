...
def do_deploy(archive_path):
    ...
    if not os.path.exists(archive_path):
        return False
    Upload = put(archive_path, remote="/tmp/")
    if Upload.failed:
        return False
    archive = archive_path[9:-4]
    Make_dir = run("mkdir -p /data/web_static/releases/'{0}'".format(archive))
    if Make_dir.failed:
        return False
    Uncompress = run(
        "tar -xzf '{0}''{1}'.tgz -C /data/web_static/releases/'{1}'".
        format(remote, archive))
    if Uncompress.failed:
        return False
    Delete_compressed = run("rm '{}''{}'.tgz".format(remote, archive))
    if Delete_compressed.failed:
        return False
    Move_uncompressed = run(
        "mv /data/web_static/releases/'{0}'/web_static/* \
        /data/web_static/'{0}'/".format(archive))
